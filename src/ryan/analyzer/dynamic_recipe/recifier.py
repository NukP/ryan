...
def generate_recipe_dgpost(dgpost_recipe_dir: Path, data_folder_dir: Path) -> None:
    ...
    # electro yaml
    (dgpost_recipe_dir / "01_dynamically_generated_dgpost_electro.yaml").write_text(templates.template_dgpost_electro)

    # peis yaml
    (dgpost_recipe_dir / "02_dynamically_generated_dgpost_peis.yaml").write_text(templates.template_dgpost_peis)
...
def check_data(data_folder_dir: Path) -> Dict[str, Union[float, bool]]:
    ...
    try:
        ...
        if not output_metadata:
            raise FileNotFoundError(f"No metadata file found in {data_folder_dir} or Output/{exp_name}")
...
def generate_recipe_dgpost_gc(dict_check_data: dict) -> str:
    ...
    if dict_check_data["cathode_reaction"] == "Carbon dioxide reduction":
        ...
        raise ValueError(
            "The cathode reaction filled in the metadata is not recognized. Charge per carbon atom cannot be determined."
        )
